  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "mapa",
              fluidRow(
                box(
                  title = "Descrição do Mapa Interativo",
                  width = 12,
                  status = "primary",
                  solidHeader = TRUE,
                  p("Um ponto essencial do processo de construção desse gráfico foi o filtro aplicado, que garantiu que apenas municípios com pelo menos 5 estudantes com dados completos fossem incluídos nas regressões. Sem esse filtro, muitas regressões não poderiam ser rodadas, pois resultariam em valores NA devido à insuficiência de observações.")
                )
              ),
              fluidRow(
      ...
      tabItem(tabName = "graficos",
              fluidRow(
                box(
                  ...
                  status = "primary",
                  solidHeader = TRUE,
                  p("Na relação entre β e o PIB municipal, não se observa um padrão claro, indicando que a mobilidade educacional não varia de forma consistente com o nível de desenvolvimento econômico dos municípios. A correlação entre β e a escolaridade média das capitais brasileiras também não apresenta um padrão bem definido, embora haja uma leve tendência de menor mobilidade nos extremos da distribuição. Por fim, a correlação entre β e a pobreza mostra uma grande dispersão dos pontos, sem um padrão evidente de relação negativa ou positiva.")
                )
              ),
              fluidRow(
